...
//
//            log.info(String.valueOf(this.getNonce(cred.getAddress())));
//
//
...
//    }
    ...
    public String sendFast(String privKey, String to) throws Exception {
        Credentials cred = Credentials.create(privKey);
        BigInteger valueWei = Convert.toWei("0.000000001", Convert.Unit.ETHER).toBigInteger();
//        BigInteger valueWei = Convert.toWei(amountEth, Convert.Unit.ETHER).toBigIntegerExact();

        // комиссии
        BigInteger tip = web3j.ethMaxPriorityFeePerGas().send().getMaxPriorityFeePerGas();
        BigInteger base = web3j.ethGetBlockByNumber(DefaultBlockParameterName.PENDING, false)
                .send().getBlock().getBaseFeePerGas();

        if (base == null) base = web3j.ethGasPrice().send().getGasPrice();

        BigInteger maxFeePerGas = base.multiply(BigInteger.valueOf(2)).add(tip);

        // nonce
        BigInteger nonce = web3j.ethGetTransactionCount(
                cred.getAddress(), DefaultBlockParameterName.PENDING
        ).send().getTransactionCount();

        BigInteger gasLimit = BigInteger.valueOf(21_000);

        // собираем EIP-1559 транзакцию
        ...
        byte[] signed = TransactionEncoder.signMessage(tx, cred);
        String hex = Numeric.toHexString(signed);

        EthSendTransaction resp = web3j.ethSendRawTransaction(hex).send();
        if (resp.hasError()) {
            throw new RuntimeException("RPC error: " + resp.getError().getMessage());
        }

        return resp.getTransactionHash();
    }

//    private BigInteger getNonce(String addr) {
//        try {
